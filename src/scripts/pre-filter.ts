...
async function filterOneBatch(batch: Article[], results: Map<string, boolean>) {
  ...
  const articleList = batch.map((a, idx) => {
    ...
    const prompt = `你是保理与供应链金融领域的内容审核专家。判断以下文章标题是否属于保理/供应链金融行业的实质资讯，或直接影响该行业的内容。

【判为相关】满足之一即放行（交下游评分细化，不必严苛）：
- 直接相关：保理业务、应收账款融资/转让、供应链金融、商业保理、银行保理、融资租赁、债权转让、供应链ABS/ABN、动产融资统一登记(中登)、反向保理、保兑仓、票据/供应链票据贴现
- 间接但影响行业：账期/回款/应收账款或应付账款治理、拖欠中小企业账款治理、中小企业融资难贵、贸易金融、核心企业确权、信用流转、产融结合、普惠金融信贷
- 监管与政策：央行/金监总局/证监会/地方金融监管局发布涉及信贷、货币、普惠、供应链金融、融资租赁、建设审批、商业保理监管的动态或制度，即便标题未直接写"保理"
- 租赁行业动态：融资租赁/金融租赁业务规模、监管、项目、案例、行业报告
- 真实行业事件、案例、数据、企业动态、行业报告

【判为不相关】满足之一：
- 培训课/研修班/总裁班/招商会/峰会报名等课程或活动招生广告
- 白皮书/资料包/扫码领取/加微信/进群等引流留资(lead-gen)
- 正文无实质内容：空壳、纯导航、纯机构介绍、党建/组织内部活动
- 仅蹭"供应链""保理"等词但实际讲个股行情/IPO/纯宏观叙事/与金融供应链无关的其他行业

注意：不要因标题未出现"保理"二字就误判为无关；只要主题落在上述"间接但影响行业"或"监管政策"范围内即判相关，请放在详细打分阶段细分。只有明显无关或营销/空壳内容才判不相关。
